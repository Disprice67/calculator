...
от {BOUND_ONE}р.  и до {BOUND_TWO}р. с учетом того, 
что {MULTIPLYING}% единиц оборудования система не нашла в 
продаже на рынке. Расчет выполнен по курсу 85р/$»\n""")]
    ...
    def get_message(self, number, flag: bool = None, 
        ...
        multiplying = ceil((write.MULTIPLYING - 1) * 100)

        if number == 7 and not flag:
            return self.MESSAGE_INFO[2].format(BOUND_ONE=write.LOW_BOUND,BOUND_TWO=write.UP_BOUND, 
                                              MULTIPLYING=multiplying)
        if number == 7 and flag:
            return self.MESSAGE_INFO[2].format(BOUND_ONE=write.UP_BOUND, BOUND_TWO=write.LOW_BOUND, 
                                               MULTIPLYING=multiplying)
    ...
    def finalbody(self, writer: Writer):
        ...
        if writer.MULTIPLYING > STATIC_MULTIPLYING:
            body += self.get_message(6)
        
        if writer.LOW_BOUND < writer.UP_BOUND:
            body += self.get_message(7, False, writer)
        
        if writer.LOW_BOUND > writer.UP_BOUND:
            body += self.get_message(7, True, writer)
