...
class Writer(File):
    ...
    * method setconst() extract constants from excel
    ...
    # root_dir
    OUTPUT_FILE: str = ROOT_DIR + 'Исходящий/final.xlsx'
    ...
    def setconst(self):
        ...
        collision = ('None', None, '', 0.0, '0')

        for i in range(2, 5):
            wbxl = Book(self.OUTPUT_FILE)
            item = wbxl.sheets['Оценка рыночной стоимости'].range(f'B{i}').value
            if item in collision:
                item = 0
            
            if i == 2:
                self.MULTIPLYING = item

            elif i == 3:
                if item != 0:
                    self.LOW_BOUND = '{:,}'.format(ceil(item)).replace(',', ' ')
                else:
                    self.LOW_BOUND = item

            elif i == 4:
                if item != 0:
                    self.UP_BOUND = '{:,}'.format(ceil(item)).replace(',', ' ')
                else:
                    self.UP_BOUND = item
