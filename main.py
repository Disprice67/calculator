...
* method archivefind() trying to find the part number in the archive
...
MESSAGE = ['Хотим купить под: {atr}. По цене: {pricegrabe}$', 'P/N совпал без букв: {part}']

CATEGORY_EXCEPTION = ('LIC-1', 'SOFT-1', 'MSCL')

SEARCH = [('Свод', 'ЗИП'), ('Закупка1', 'ЗИП'), ('Закупка2', 'ЗИП'), ('Архив', '№ ПОСЛЕДНЕГО ЗАПРОСА ИЗ АРХИВА')]

# sql_request
SQL_REQUEST = {
                  'Архив': [SQL_ARCHIVE, SQL_ARCHIVE_IN, SQL_ARCHIVE_OUT, ARCHIVE],
...
def archivefind(db: Database, key, data, excel: Excel):
    """Search_archive."""

    request = db.takeinfo(SQL_REQUEST['Архив'][3].format(part=f"{excel.filterkey(key, 'PN')}"))
    if request:
        value = request[0]['№ЗАПРОСА']
        if value:
